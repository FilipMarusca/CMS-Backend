...
public class AuthorView extends BaseView {
    ...
    @FXML
    public void uploadSummaryHandler() {
        ...
        try {
            ...
            pdfDataSummary = new byte[(int) file.length()];
            DataInputStream dis = new DataInputStream(new FileInputStream(file));
            dis.readFully(pdfDataSummary);  // read from file into byte[] array
            dis.close();
    ...
    void submitPaper() {
        ...
        if (pdfDataSummary == null || pdfDataSummary.length == 0) {
            ShowAlert.showAlert("No PDF summary uploaded");
            return;
        }
        ...
        try {
            ...
            for (Paper p : controller.getAllPapers()) {
                if (p.getTitle().equals(title) && p.getAuthor().getUsername().equals(loggedUser.getUsername()) && p.getEdition().getId() == edition.getId()) {
                    ...
                    Paper paper = new Paper(author, edition, PaperStatus.InReview, title, topic, pdfData, pdfDataSummary);
                    p.setTopic(topic);
                    p.setPaperPDF(pdfData);
                    p.setSummaryPDF(pdfDataSummary);
